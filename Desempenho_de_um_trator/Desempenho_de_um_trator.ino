...
int cont_vazao_s = 0, cont_vazao_e = 0, i = 0, j = 0;

String gravacao = "", vetGPS[15], vetBalanca[19], p_gps;

char crt_gps, crt_balanca;
...
void loop() {
  ...
  if (Serial2.available() > 0) {
    crt_balanca = (byte)Serial2.read();
    //Serial.println(crt_balanca);
    if (j == 4 || j == 5 || j == 6 || j == 7 || j == 8 || j == 9) {
      //Serial.println(crt_balanca);
      vetBalanca[j] = crt_balanca;
      j++;
    } else {
      if (j != 19) {
        j++;
      } else {
        //leituraBalanca = vetBalanca[4] + vetBalanca[5] + vetBalanca[6] + vetBalanca[7] + vetBalanca[8] + vetBalanca[9];
        //Serial.println(leituraBalanca);
        //leituraBalanca = "";
        j = 0;
      }
    }
  ...
  gravacao = String(VL_dianteira_esquerda) + ";" + String(VL_dianteira_direita) + ";" + 
  String(VL_traseira_esquerda) + ";" + String(VL_traseira_direita) + ";" + String(vazao_e) + ";" + String(vazao_s) + ";" +
  String(String(vetBalanca[4]) + String(vetBalanca[5]) + String(vetBalanca[6]) + String(vetBalanca[7]) + String(vetBalanca[8]) + String(vetBalanca[9]) + ';' + "xxx" + ";" + 
  vetGPS[1]) + ";" + String(vetGPS[2]) + ";" + String(vetGPS[3]) + ";" + String(vetGPS[4]) + ";" + String(vetGPS[5]) + ";" + String(vetGPS[6]) + ";" + String(vetGPS[9]) + ";" + String(vetGPS[10]);  ;
  Gravacao();
}
